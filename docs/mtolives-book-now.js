/*! mtolives-book-now v0.4.6 (final)
   - Web Component (Shadow DOM) date-range picker using Flatpickr (rangePlugin).
   - Brand tokens: --accent (endpoints), --range (in-range tint), --hover-day (true hover).
   - Features: labels vs placeholders, intent pill, focus pointer, min/max nights,
               neutral 1-night bridge (no default blue), local vendor + CDN fallback.
   - Handoff: BOOK NOW navigates with ?checkin=YYYY-MM-DD&checkout=YYYY-MM-DD.
   - MIT License (c) 2025 Mount of Olives Hotel Ltd.
*/
(() => {
  ...
  // Global guard: if a calendar ever leaks to <body>, keep polish consistent
  ...
  class MtOlivesBookNow extends HTMLElement {
    ...
    static get observedAttributes() {
      return [
        'book-url', 'show-months', 'popup', 'display-format',
        'min-nights', 'max-nights', 'align', 'labels',
        'accent', 'rounded',
    ...
    // Centralized text/format accessors (live off attributes)
    get i18n() {
    ...
    constructor() {
      ...
      const labels = (this.getAttribute('labels') || 'none').toLowerCase(); // 'show'|'hidden'|'none'
      ...
      this.shadowRoot.innerHTML = `
        ...
        <style>
          :host{
            --accent:${accent};
            /* Token split: keep legacy --hover as alias to --range for back-compat */
            --hover: rgba(128,128,0,.24);
            --range: var(--hover);
            --hover-day: rgba(0,0,0,.06);
            --fieldW: 250px;
            --rounded: ${rounded};
          ...
          *,*::before,*::after{ box-sizing:border-box; }
          ...
          /* --- Flatpickr theming inside Shadow DOM --- */
          ...
          /* True day hover (non-selected, non-range days) */
          ...
          /* Neutralize Flatpickr's default blue endpoint connectors globally */
          .flatpickr-day.startRange,
          .flatpickr-day.endRange{
            box-shadow: none !important;
          }

          /* Focus pointer that tracks active field */
          .mto-pin{
        ...
        <div class="bar ${labels === 'none' ? 'hideLabel' : ''}">
          ...
          <button id="book" disabled>BOOK NOW</button>
        </div>
      `;
    }

    // Apply labels/visibility/placeholders from attributes
    applyLabels() {
    ...
    // Mirror selected dates into inputs using display-format
    #mirrorInputs(inst = this.#fp) {
      const btn = this.shadowRoot.getElementById('book');
      if (!inst || !this.#inputIn || !this.#inputOut) { if (btn) btn.disabled = true; return; }
    ...
    async init() {
      ...
      // Wait for Shadow-DOM CSS to be readable before creating the picker
      ...
      // Remember which field opened the calendar (affects intent pill & pointer)
      let openedBy = 'in';
      ...
      // Pointer that tracks active field
      ...
      // One-night bridge helpers (selection + hover preview)
      ...
      this.#fp = flatpickr(this.#inputIn, {
        ...
        onOpen: (_d, _s, inst) => {
          ...
          // Wire hover preview once per calendar
          const c = inst.calendarContainer;
          if (!c._mtoHoverWired) {
            ...
            if (days) {
              ...
              days.addEventListener('mouseleave', () => { setOneNightPreview(inst, null); });
            }
            c._mtoHoverWired = true;
          }
        },

        // Reduce chevron/header flash on cold load
        onReady: (_d, _s, inst) => {
          ...
          if (link && !link.sheet) link.addEventListener('load', show, { once: true }); else show();
        },

        onChange: (dates, _str, inst) => {
          if (mutating) return;

          // Define helper before first use (lexical this)
          ...
          if (dates.length === 1) {
            this.#mirrorInputs(inst);
            setOneNightClass(inst);        // clears class for d.length === 1
          ...
          // Valid range
          this.#mirrorInputs(inst);
          setOneNightClass(inst);        // adds class if nights === 1
          setOneNightPreview(inst, null);
      ...
      btn.addEventListener('click', () => {
        ...
        location.href = `${url}${url.includes('?') ? '&' : '?'}${q.toString()}`;
      });

      // Reflect alignment preference as attribute (for CSS grid justification)
